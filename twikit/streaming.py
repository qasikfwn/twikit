...
class StreamingSession:
    ...
    async def update_subscriptions(
        self, subscribe: set[str] | None = None, unsubscribe: set[str] | None = None
    ) -> Payload:
        """
        ...
        .Topic
        """
        return await self._client._update_subscriptions(self, subscribe, unsubscribe)
...
def _event_from_data(name: str, data: dict) -> StreamEventType:
    if name == 'config':
        ...
        return ConfigEvent(session_id, subscription_ttl_millis, heartbeat_millis)
    ...
    if name == 'tweet_engagement':
        ...
        return TweetEngagementEvent(
            like_count, retweet_count, view_count, view_count_state, quote_count, reply_count
        )
...
def _payload_from_data(data: dict) -> Payload:
    events = {name: _event_from_data(name, data) for (name, data) in data.items()}
...
StreamEventType = (
    ConfigEvent | SubscriptionsEvent | TweetEngagementEvent | DMTypingEvent | DMTypingEvent
)
